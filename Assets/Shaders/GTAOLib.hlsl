...
half Classical_SSAO(float2 uv, half rcpSamleCount, half3 norm_o, half3 vpos_o, float depth_o)
{
    half ao = 0.0;
    ...
    for(int s = 0; s < SAMPLE_COUNT; s++)
    {
        ...
        v_s1 *= sqrt((half(s) + half(1.0)) * rcpSamleCount) * RADIUS;
    ...
    ao *= RADIUS;
    ao = PositivePow(ao * INTENSITY * rcpSamleCount, kContrast);
...
half4 SSAO(Varyings input) : SV_Target
{
    ...
    half3 vpos = GetPosition2(uv);
    half3 viewDir = normalize(0- vpos);
    half3 norm_v = GetNormal(uv);

    half2 radius_thickness = lerp(half2(RADIUS, 1), half2(0, 0), ComputeDistanceFade(vpos.b).xx);
    ...
    half stepRadius = (max(min((radius* 31)/ vpos.b, 512), (half)SLICE) / ((half)SLICE + 1));

    half angle, BentAngle, wallDarkeningCorrection, sliceLength, n, cos_n;
    ...
    const half rcpSamleCount = half(rcp(SAMPLE_COUNT));
    half ao = 0.0;
    ...
    {
        angle = (i + 30 * noiseDirection) * (PI / (half)SAMPLE_COUNT);
        ...
        sliceNormal = norm_v - planeNormal * dot(norm_v, planeNormal);
        ...
        {
            uvOffset = (sliceDir.xy * (1 / (SCREEN_PARAMS.xy * DOWNSAMPLE))) * max(stepRadius * (j + noiseOffset), j+1);
            uvSlice = uv.xyxy + float4(uvOffset.xy, -uvOffset);

            h1 = GetPosition2(uvSlice.xy) - vpos;
            h2 = GetPosition2(uvSlice.zw) - vpos;
       ...
       ao += sliceLength * IntegrateArc_CosWeight(h, n) * rcpSamleCount;
    }

    ao  = 1 - ao;
    ao = PositivePow(ao* INTENSITY, 2.5);
    #else
    ao = Classical_SSAO(uv, rcpSamleCount, norm_o, vpos_o, depth_o);
    #endif
